...
export async function seedLevelingCosts(prisma: PrismaClient, logger: Logger) {
  logger.info('Seeding Leveling Costs...');
  ...
  logger.info(`Seeded ${levelingCostData.length} Leveling Cost records.`);
  logger.info('Finished seeding Leveling Costs.');
}