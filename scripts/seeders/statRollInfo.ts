...
// Data approved in the visual table, with calculated target rolls
const statRollData = [
  // --- Critical Chance ---
...
export async function seedStatRollInfo() {
  console.log('Seeding Stat Roll Info...');

  // 1. Get all stats from the DB
  ...
  const statRollInfoData = statRollData.map(roll => {
    ...
    if (!statId) {
      console.warn(`Stat "${roll.statName}" not found in the database. Skipping.`);
      return null;
    }
  ...
  }).filter(Boolean); // Filter out any nulls from stats not found
  ...
  if (statRollInfoData.length > 0) {
    ...
    console.log(`Seeded ${statRollInfoData.length} Stat Roll Info records.`);
  } else {
    console.log('No new Stat Roll Info records to seed.');
  }

  console.log('Finished seeding Stat Roll Info.');
}
